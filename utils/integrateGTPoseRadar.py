...
out_folder = "/home/ajay/ARPG/RMap/Grad-PU/output/2023-06-23T16:54:19.443319/test/ec_run0_radar_RScan_16_GTPose"
...
for topic,msg,time in bag.read_messages(topics=["/lidar_ground_truth"]):
    ...
    gt_odom.append(np.linalg.inv(transformation_matrix))
...
for topic, msg, time in bag.read_messages(topics=["/mmWaveDataHdl/RScan"]):
    ...
    pcd_transform = pcd_transform.transform(gt_odom[closest_idx])
    total_points.extend(pcd.points)
    o3d.io.write_point_cloud(os.path.join(out_folder, pcd_files[index]), pcd_transform, True, True)

    index += 1
point_cloud = o3d.geometry.PointCloud()
point_cloud.points = o3d.utility.Vector3dVector(np.array(total_points))
o3d.io.write_point_cloud(os.path.join(out_folder, "final_pcd.ply"), point_cloud, True, True)
